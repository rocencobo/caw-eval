...
def load_judge_results(path: str) -> dict[str, dict[str, Any]]:
    ...
    if isinstance(raw, list):
        return {
            entry["trace_id"]: {**entry, "available": True} for entry in raw if "trace_id" in entry
        }
...
def build_score_comment(
    ...
    return f"{stage} ({scoring_source}) | {stage_score:.2f} = {formula}" if formula else f"{stage} ({scoring_source}) | {stage_score:.2f}"
    ...
    for dim_key, score_name in _dim_score_names.items():
        ...
        if dim is not None:
            scores_to_upload.append(
                (score_name, dim.score, f"[{dim.method}] {dim_key}={dim.score:.2f} — {dim.reasoning}")
            )
...
def session_main() -> None:
    ...
    dataset_items_cache: dict[str, tuple[dict, dict, dict]] = {}  # item_id -> (input, expected, metadata)
    if not args.item_id and session_path.is_dir():
        try:
            ...
            if dataset_items_cache:
                print(f"[INFO] Loaded {len(dataset_items_cache)} items from dataset {args.dataset_name}")
        except Exception as e:
